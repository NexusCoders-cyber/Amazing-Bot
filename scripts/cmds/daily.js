...
export default {
    config: {
        ...
        author: 'Raphael Ilom',
        version: '2.0',
        shortDescription: 'Claim daily coins with a streak bonus',
        category: 'economy',
        coolDown: 3,
        role: 0,
    ...
    async onStart({ sender, reply }) {
        ...
        reply(
            `🎁 *Daily Claimed!*\n\n` +
            `Earned : ${fmtCoins(total)}  (base ${fmtCoins(base)} + streak +${fmtCoins(bonus)})\n` +
            `Streak : ${newStreak} day${newStreak !== 1 ? 's' : ''}\n` +
            `Wallet : ${fmtCoins(newWallet)}\n\n` +
            (newStreak >= 7 ? '🔥 7-day streak bonus active!' : `Streak ${newStreak}/7 for max bonus`)
        );
    },
};
