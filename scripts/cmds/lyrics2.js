...
export default {
    ...
    async onStart({ args, reply, prefix, React }) {
        ...
        const q = args.join(' ');
        try { const { default: axios } = await import('axios'); const { data } = await axios.get(`https://broken-api-production-31d5.up.railway.app/api/tools/lyrics`, { timeout: 30000 }); reply(`🎵 *${data.title}* — ${data.author}\n\n${data.lyrics?.substring(0, 1900) || 'No lyrics found'}`); } catch { reply(`❌ Could not find lyrics for "${q}"`); }
    },
};
