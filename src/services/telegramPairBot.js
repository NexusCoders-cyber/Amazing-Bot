...
function commandShortcutButtons() {
    return {
        keyboard: [
            [{ text: '/pair YOUR_NUMBER' }],
            [{ text: '/pairs' }, { text: '/delpair' }],
            [{ text: '/help' }, { text: '/buttons' }],
            [{ text: '/admins' }, { text: '/addadmin TELEGRAM_ID' }],
...
function menuKeyboard() {
    return {
        keyboard: [
            ...
            [{ text: '/tts Hello from the bot' }],
            [{ text: '/ping' }, { text: '/uptime' }],
            [{ text: '/owners' }, { text: '/admins' }],
            [{ text: '/addadmin TELEGRAM_ID' }, { text: '/menu' }]
        ],
        resize_keyboard: true,
        one_time_keyboard: false
    };
...
async function fetchTelegramPlayAudio({ query, video }) {
    const headers = { 'User-Agent': 'Mozilla/5.0 (ILOM-Bot Telegram)' };
    const endpoints = [
        {
            name: 'DrexApp',
...
${rows.join('\n')}`);
    ...
    const handleCallbackQuery = async (update) => {
        ...
        if (action === 'act_pair') {
            ...
            return sendText(chatId, '📱 Send your WhatsApp number or LID (country code included). Example: YOUR_NUMBER_OR_LID');
        }
