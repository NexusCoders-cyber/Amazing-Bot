        ...
        try {
            ...
            if (quoted?.imageMessage) {
                const imageBuffer = await downloadMediaMessage(
                    { message: { imageMessage: quoted.imageMessage }, key: quotedKey },
                    'buffer',
                    {}
                );
                ...
                const videoBuffer = await downloadMediaMessage(
                    { message: { videoMessage: quoted.videoMessage }, key: quotedKey },
                    'buffer',
                    {}
                );
            ...
            await sock.sendMessage(from, { text: `❌ Failed to save: ${error.message}` }, { quoted: message });
        }
    }
};
