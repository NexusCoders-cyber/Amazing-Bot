...
export default {
  ...
  async onStart({ args, from, reply }) {
    const state = args[0] ? !['off','false','0','disable'].includes(args[0].toLowerCase()) : !((await threadsData.getSetting(from, 'antiforeign')) ?? false);
    await threadsData.setSetting(from, 'antiforeign', state);
