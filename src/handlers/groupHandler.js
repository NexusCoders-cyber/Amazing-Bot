...
let createPromoteImage = null;
let createDemoteImage = null;

async function loadCanvasUtils() {
    if (createPromoteImage) return;
    try {
        const mod = await import('../utils/canvasUtils.js');
        createPromoteImage = mod.createPromoteImage;
        createDemoteImage = mod.createDemoteImage;
    } catch {
        createPromoteImage = async () => null;
        createDemoteImage = async () => null;
    }
}
    ...
    async handleParticipantsUpdate(sock, groupUpdate) {
        try {
            ...
            if (action === 'add') {
                ...
                if (config.events?.groupPromote) await this.handleGroupPromote(sock, groupUpdate);
                this.updateStats(id, 'updates', 1);
            } else if (action === 'demote') {
                if (config.events?.groupDemote) await this.handleGroupDemote(sock, groupUpdate);
                this.updateStats(id, 'updates', 1);
            }
        } catch (error) {
