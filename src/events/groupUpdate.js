    ...
    try {
        ...
        if (action === 'promote' && participants.length > 0) {
            for (const participant of participants) {
                try {
                    ...
                    if (promoteImage) {
                        await sock.sendMessage(groupId, { image: promoteImage, caption: promoteMessage, mentions });
                    } else {
        ...
        if (action === 'demote' && participants.length > 0) {
            for (const participant of participants) {
                try {
                    ...
                    if (demoteImage) {
                        await sock.sendMessage(groupId, { image: demoteImage, caption: demoteMessage, mentions });
                    } else {
        ...
        if (update.subject) {
            try {
                ...
                if (updateImage) {
                    await sock.sendMessage(groupId, { image: updateImage, caption: updateMessage, mentions });
                } else {
